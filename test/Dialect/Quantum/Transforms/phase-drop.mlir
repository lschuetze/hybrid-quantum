// RUN: quantum-opt --quantum-optimise %s -split-input-file | FileCheck %s
...
func.func @drop_z_before_measure() -> !quantum.qubit<1> {
  ...
  // CHECK-DAG: %[[MEAS:.+]], %[[QOUT:.+]] = "quantum.measure"(%[[Q1]]) : (!quantum.qubit<1>) -> (!quantum.measurement<1>, !quantum.qubit<1>)
  %m, %qout = "quantum.measure"(%q2) : (!quantum.qubit<1>) -> (!quantum.measurement<1>, !quantum.qubit<1>)
...
func.func @drop_z_before_measure_multiqubit() -> !quantum.qubit<3> {
  ...
  // CHECK-DAG: %[[MEAS:.+]], %[[QOUT:.+]] = "quantum.measure"(%[[Q]]) : (!quantum.qubit<3>) -> (!quantum.measurement<3>, !quantum.qubit<3>)
  %m, %qout = "quantum.measure"(%qz) : (!quantum.qubit<3>) -> (!quantum.measurement<3>, !quantum.qubit<3>)
...
func.func @no_drop_z_interleaved() -> !quantum.qubit<2> {
  ...
  // CHECK-DAG: %[[M:.+]], %[[QOUT:.+]] = "quantum.measure"(%[[QH]]) : (!quantum.qubit<2>) -> (!quantum.measurement<2>, !quantum.qubit<2>)
  %m, %qout = "quantum.measure"(%qh) : (!quantum.qubit<2>) -> (!quantum.measurement<2>, !quantum.qubit<2>)
...
func.func @no_drop_z_with_merge() -> !quantum.qubit<2> {
  ...
  // CHECK-DAG: %[[M:.+]], %[[QOUT:.+]] = "quantum.measure"(%[[QMERGED]]) : (!quantum.qubit<2>) -> (!quantum.measurement<2>, !quantum.qubit<2>)
  %m, %qout = "quantum.measure"(%qmerged) : (!quantum.qubit<2>) -> (!quantum.measurement<2>, !quantum.qubit<2>)
...
// CHECK-LABEL: func.func @drop_S_before_measure(
func.func @drop_S_before_measure() -> !quantum.qubit<1> {
  // CHECK-DAG: %[[Q1:.+]] = "quantum.alloc"() : () -> !quantum.qubit<1>
  %q1 = "quantum.alloc"() : () -> (!quantum.qubit<1>)
  // CHECK-NOT: "quantum.S"
